...
export const getAverageInternalTeamActivity = async (
  ...
  try {
    const res = await fetch(
      `http://18.237.25.116:8000/average-internal-team-activity?${clientQueryParam}&${agentsQueryParam}&${teamLeadQueryParam}&${OM_QueryParam}&${CSM_QueryParam}&startdate=${startingDateFilter}&enddate=${endingDateFilter}`,
      {
        headers: {
...
const AverageInternalTeamActivity = () => {
  ...
  return (
    <>
      {data[0].average_of_average_activity === null
        ? 0
        : data[0].average_of_average_activity?.toFixed(2) || 0}
